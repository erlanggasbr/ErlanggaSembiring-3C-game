...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Move(Vector2 axisDirection)
    {
        ...
        else if (isPlayerClimbing)
        {
            ...
            Vector3 checkerLeftPosition = transform.position + (transform.up * 1) + (-transform.right * .5f);
            Vector3 checkerRightPosition = transform.position + (transform.up * 1) + (transform.right * .5f);
            Vector3 checkerUpPosition = transform.position + (transform.up * 1.9f);
            Vector3 checkerDownPosition = transform.position + (-transform.up * .2f);
        ...
        else if (isPlayerGliding)
        {
            ...
            rotationDegree.z += _glideRotationSpeed.z * axisDirection.x * -1 * Time.deltaTime;
            //
    ...
    private void Sprint(bool isSprint)
    {
        ...
        if (isSprint && !isPlayerCrouch && _isGrounded)
        {
    ...
    //{
    //    Debug.DrawLine(transform.position + (Vector3.up * 1.8f), transform.position + (Vector3.up * 2f), Color.blue);
    //}
